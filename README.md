This project is a pilot project and develops a scoring solution for matrix-based math practices.
Backend: Python 3 + Flask
Frontend: React  (HTML + JavaScript for alpha version)
The key technical challenge is recognizing handwritten two-digit numbers written by young students in high-time pressure with CNN. This application also handles skewed and/or distorted images. 
This app uses Mongo DB to manage the uses, forms, scoring information. 
The Alpha version uses Flask-Login to manage the authentication with SQLalchemy. If the front end is transferred to React, React should manage the authentication.
